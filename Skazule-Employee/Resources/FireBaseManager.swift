...
//  Skazule-Employee
//
//  Created by Anita Singh on 7/20/23.
//
...
//                saveFirebaseUserId(userId: userId)
                ...
                self.fdbRef.child("Users").child(userId).updateChildValues(userDict as [AnyHashable : Any])
                onSuccess(true)
            }
        }
    }
...
//                self.fdbRef.child("Users").child(employeeId).updateChildValues(userDict as [AnyHashable : Any])
//                onSuccess(true)
//            }
//        }
//    }
    ...
    func addEmployeeLoginWithFirebase(email: String, password: String, userName: String, role: String, timestamp: String, profilePic: String, fcmKey: String?, checkedUnCheck: String?, alreadyExistEmployee: Bool, onSuccess:@escaping(Bool) -> Void, onError:@escaping(String) -> Void){
        ...
        Auth.auth().signIn(withEmail: email, password: "123456") { (result, error) in
            ...
            if error != nil{
                ...
                {
                    self.employeeIdArray.append(employeeId)   //.add(userId!!)
                }
                else
                {
                    if let index = self.employeeIdArray.firstIndex(of: employeeId) {
                        self.employeeIdArray.remove(at: index)
                    }
                }
  //              let chatViewModel = ChatViewModel()
 //               chatViewModel.createChatRoom()
...
//                fdbRef.child("Users").child(employeeId).updateChildValues(userDict as [AnyHashable : Any])
                self.fdbRef.child("Users").child(employeeId).setValue(userDict as [AnyHashable : Any])
